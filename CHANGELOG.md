...
### Added
- **SyntheticDiD `variance_method="bootstrap_refit"`** — paper-faithful Arkhangelsky et al. (2021) Algorithm 2 step 2. On each pairs-bootstrap draw, re-estimates ω̂_b and λ̂_b via two-pass sparsified Frank-Wolfe on the resampled panel, using the fit-time normalized-scale zeta. Opt-in; default remains `"placebo"`. Expected wall-clock ~10–100× slower per fit than the fixed-weight shortcut (panel-size dependent). Survey designs (including pweight-only) composed with refit raise `NotImplementedError` in this release — Rao-Wu rescaled weights composed with Frank-Wolfe re-estimation is tracked as a follow-up. BusinessReport inference-label, `result.summary()`, `result.n_bootstrap`, and the `SyntheticDiD` power-analysis guidance strings all recognize the new enum value.
