...
### Added
- **SyntheticDiD `variance_method="bootstrap_refit"`** — paper-faithful Arkhangelsky et al. (2021) Algorithm 2 step 2. On each pairs-bootstrap draw, re-estimates ω̂_b and λ̂_b via two-pass sparsified Frank-Wolfe on the resampled panel, using the fit-time normalized-scale zeta. This also matches the behavior of R's default `synthdid::vcov(method="bootstrap")`, which stores `update.omega=TRUE` in `attr(estimate, "opts")` and rebinds those opts back into `synthdid_estimate` inside its bootstrap loop so the renormalized ω serves only as Frank-Wolfe initialization. Opt-in; default remains `"placebo"`. Expected wall-clock ~10–100× slower per fit than the existing fixed-weight shortcut (panel-size dependent). Survey designs (including pweight-only) composed with refit raise `NotImplementedError` in this release — Rao-Wu rescaled weights composed with Frank-Wolfe re-estimation is tracked as a follow-up. BusinessReport inference-label, `result.summary()`, `result.n_bootstrap`, and the `SyntheticDiD` power-analysis guidance strings all recognize the new enum value. **Note:** the existing `variance_method="bootstrap"` is now documented in REGISTRY.md as a deviation from R's default — our fixed-weight shortcut is faster but under-estimates SE relative to the paper's Algorithm 2 and to R's default bootstrap. The coverage calibration table below quantifies the gap; the R-parity test fixture (`test_bootstrap_se_matches_r`) is a bit-identity check against a manual fixed-weight invocation of `synthdid_estimate()` that omits the opts rebind, not against R's default `vcov` behavior.
- **SyntheticDiD coverage Monte Carlo calibration table** (`docs/methodology/REGISTRY.md` §SyntheticDiD) — rejection rates at α ∈ {0.01, 0.05, 0.10} across all four variance methods on 3 representative DGPs (balanced / exchangeable, unbalanced, and Arkhangelsky et al. (2021) AER §6.3 non-exchangeable). Artifact at `benchmarks/data/sdid_coverage.json` (500 seeds × B=200), regenerable via `benchmarks/python/coverage_sdid.py`. Quantifies the fixed-weight deviation and supports the documented anti-conservatism claim.
