...
- **Note (survey + jackknife composition):** PSU-level leave-one-out with stratum aggregation (Rust & Rao 1996). For a design with strata `h = 1..H` and PSUs `j = 1..n_h` within each stratum:
  ...
  **Degenerate LOO handling** (skip, don't raise): (a) LOO removes all treated units (e.g., all treated in one PSU) → skip this `j`; (b) `ω_eff_kept.sum() <= 0` after composition → skip; (c) `w_treated_kept.sum() <= 0` → skip. Strata with `n_h < 2` are silently skipped (stratum-level variance unidentified). If every stratum is skipped, returns `SE=NaN` with a `UserWarning`. PSU-None designs: each unit is treated as its own PSU within its stratum (matches the implicit-PSU convention established in PR #355 R8 P1). Unstratified single-PSU short-circuits to `SE=NaN`.
