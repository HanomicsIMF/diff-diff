...
- Alternative: Bootstrap at unit level (matching R's `synthdid::vcov(method="bootstrap")`)
  ...
  6. `SE = sqrt((r-1)/r) × sd(bootstrap_estimates, ddof=1)` where `r = n_successful`, matching R's `vcov.R` and the placebo formula above (equivalent to Arkhangelsky et al. (2021) Algorithm 2's `σ̂² = (1/r) Σ (τ_b − τ̄)²`)

- Alternative: Bootstrap (refit; paper-faithful): `variance_method="bootstrap_refit"` — Arkhangelsky et al. (2021) Algorithm 2 step 2 verbatim. On each pairs-bootstrap draw:
...
- **Note (R-parity test scope):** `test_bootstrap_se_matches_r` pins R's bootstrap index matrix via a JSON fixture (`tests/data/sdid_bootstrap_indices_r.json`, regenerated by `benchmarks/R/generate_sdid_bootstrap_parity_fixture.R`) and feeds it through the Python bootstrap loop via a hidden `_bootstrap_indices` kwarg. That eliminates the cross-language RNG gap (Python's PCG64 ≠ R's Mersenne Twister) and isolates the deterministic math — per-draw SDID estimator, weight renormalization, SE aggregation — which matches at 1e-10 tolerance. The test does not verify that independently-seeded R and Python bootstraps produce equal SE at any finite B; they won't.
...
- [x] Bootstrap: fixed weights (original lambda unchanged, omega renormalized for resampled controls). This matches R's `synthdid::vcov(method="bootstrap")` but is a shortcut relative to Arkhangelsky et al. (2021) Algorithm 2 step 2, which specifies re-estimating ω̂_b and λ̂_b per draw; fixed-weight bootstrap underestimates SE relative to the paper's procedure by ignoring weight-estimation uncertainty. Use `variance_method="placebo"` (Algorithm 4) or `variance_method="bootstrap_refit"` for paper-faithful inference.
- [x] Bootstrap (refit): re-estimates ω̂_b and λ̂_b per draw via two-pass sparsified Frank-Wolfe on the resampled panel (Arkhangelsky et al. 2021 Algorithm 2 step 2 verbatim). Opt-in via `variance_method="bootstrap_refit"`. Uses the fit-time normalized-scale zeta. Composed with any survey design raises `NotImplementedError` in this release.
