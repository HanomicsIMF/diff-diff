...
- Alternative: Bootstrap at unit level — **fixed-weight shortcut** (`variance_method="bootstrap"`)
  ...
  5. Compute SDID estimate with renormalized ω and original λ — no Frank-Wolfe re-estimation
  6. `SE = sqrt((r-1)/r) × sd(bootstrap_estimates, ddof=1)` where `r = n_successful` (equivalent to Arkhangelsky et al. (2021) Algorithm 2's `σ̂² = (1/r) Σ (τ_b − τ̄)²`)

  **Note (deviation from R):** this path is **not** the behavior of R's default `synthdid::vcov(method="bootstrap")`. Tracing the R source: `synthdid_estimate()` stores `update.omega = TRUE` in `attr(estimate, "opts")`, and `vcov.R::bootstrap_sample` rebinds those `opts` inside its `do.call` back into `synthdid_estimate`, so the renormalized `ω` is used **only as an initialization** for Frank-Wolfe (the `sum_normalize` helper in R's source calls this out explicitly). R's default bootstrap is therefore refit. To match R's default behavior use `variance_method="bootstrap_refit"` (below). This library's `"bootstrap"` option is a deliberate fixed-weight shortcut because it is much faster than refit and is what most users want when they pick "bootstrap" on a library where `"bootstrap_refit"` is a separate explicit choice; its coverage calibration is documented in the Notes below.

- Alternative: Bootstrap (refit; paper-faithful): `variance_method="bootstrap_refit"` — Arkhangelsky et al. (2021) Algorithm 2 step 2 verbatim, and also the behavior of R's default `synthdid::vcov(method="bootstrap")`. On each pairs-bootstrap draw:
  1. Resample ALL units (control + treated) with replacement (identical to fixed-weight step 1)
  2. Identify which resampled units are control vs treated
  ...
  6. Same SE formula as the fixed-weight path (step 6 above)

  Opt-in (default remains `"placebo"`). Expected wall-clock ~10–100× slower per fit than the fixed-weight shortcut (panel-size dependent; Frank-Wolfe second-pass can hit its 10K-iter cap on larger panels). Per-draw Frank-Wolfe non-convergence UserWarnings are suppressed inside the loop and aggregated into a single summary warning emitted after the loop when the share of valid bootstrap draws with any non-convergence event (counted once per draw — each draw runs Frank-Wolfe once for ω and once for λ, and any of those calls firing a non-convergence warning trips the draw) exceeds 5% of `n_successful` (same threshold as the retry-exhaustion guard). Composed with any survey design (including pweight-only) this path raises `NotImplementedError` in the current release — Rao-Wu rescaled weights composed with FW re-estimation needs a separate derivation (paper is un-survey; R has no survey support).
...
- **Note:** P-value computation is variance-method dependent. Placebo (Algorithm 4) uses the empirical null formula `max(mean(|placebo_effects| ≥ |att|), 1/(r+1))` because permuting control indices generates draws from the null distribution (centered on 0). Bootstrap (Algorithm 2, both fixed-weight and refit), and jackknife (Algorithm 3) use the analytical p-value from `safe_inference(att, se)` (normal-theory): bootstrap draws are centered on `τ̂` (sampling distribution of the estimator) and jackknife pseudo-values are not null draws, so the empirical null formula is invalid for them. This matches R's `synthdid::vcov()` convention, where variance is returned and inference is normal-theory from the SE.
- **Note (fixed-weight bootstrap calibration):** the `variance_method="bootstrap"` path is anti-conservative under H0 because it holds ω and λ fixed rather than re-estimating per draw. Monte Carlo on the default panel fixture (`_make_panel(att=0.0)`, n=500 seeds, B=200) returns median p-value ≈ 0.38 and empirical rejection rates of {α=0.01: 0.12, α=0.05: 0.18, α=0.10: 0.25, α=0.20: 0.34}. That's roughly 3.7× the nominal 5% rate — substantial over-rejection, consistent with the SE under-estimate from ignoring weight-estimation uncertainty. Users who need nominally-calibrated inference should prefer `variance_method="placebo"` (Algorithm 4 re-estimates both ω and λ per draw) or `variance_method="bootstrap_refit"` (paper-faithful Algorithm 2 step 2). The characterization regression test is `TestPValueSemantics::test_bootstrap_p_value_null_calibration` (slow); see also the coverage MC calibration table below for per-method rejection rates across 3 representative DGPs.
- **Note (coverage Monte Carlo calibration):** `benchmarks/data/sdid_coverage.json` carries empirical rejection rates across all four variance methods on 3 representative null-panel DGPs (500 seeds × B=200, regenerable via `benchmarks/python/coverage_sdid.py`). Under H0 the nominal rejection rate at each α equals α; rates substantially above α indicate anti-conservatism, rates below indicate over-coverage.
    ...
    Reading: **`bootstrap_refit` is close to nominal across all three DGPs**; rates track α to within Monte Carlo noise at 500 seeds (SE ≈ 0.01 at α=0.05). **`bootstrap` (fixed-weight) over-rejects** by roughly 1.8–3.2× on small N × short T panels — the largest deviation appears on the balanced DGP where the fit has the least effective sample size. **`placebo`** is well-calibrated on all three DGPs, consistent with Algorithm 4's per-permutation ω/λ re-estimation. **`jackknife`** is slightly anti-conservative on the smaller panels (balanced, AER §6.3), in line with Arkhangelsky et al. (2021) §6.3's reported 98% coverage (iid) and 93% coverage (AR(1) ρ=0.7). The `mean SE / true SD` column compares mean estimated SE to the empirical sampling SD of τ̂ across seeds; refit lands at 1.05–1.11 (well-calibrated, mildly conservative), fixed-weight at 0.85–0.94 (under-estimate).

    The schema smoke test is `TestCoverageMCArtifact::test_coverage_artifacts_present`; regenerate the JSON via `python benchmarks/python/coverage_sdid.py --n-seeds 500 --n-bootstrap 200 --output benchmarks/data/sdid_coverage.json` (~40 min on 2026-04-22 M-series Mac, Rust backend).

- **Note (R-parity test scope):** `test_bootstrap_se_matches_r` pins R's bootstrap index matrix via a JSON fixture (`tests/data/sdid_bootstrap_indices_r.json`, regenerated by `benchmarks/R/generate_sdid_bootstrap_parity_fixture.R`) and feeds it through the Python bootstrap loop via a hidden `_bootstrap_indices` kwarg. The fixture generator invokes `synthdid_estimate(weights = weights_boot)` **without** rebinding `attr(tau_hat, "opts")`; because `update.omega`/`update.lambda` default to `is.null(weights$*)` and the passed weights are non-null, this manual invocation runs the fixed-weight path — **not** R's default `vcov.synthdid_estimate(method="bootstrap")`, which does rebind `opts` and therefore runs refit (see the deviation Note on the fixed-weight alternative above). The parity test is therefore a bit-identity check between the Python fixed-weight path and a manual fixed-weight invocation of the R estimator (per-draw SDID estimator, weight renormalization, SE aggregation), which matches at 1e-10 tolerance. It does **not** anchor our `"bootstrap"` against R's default `vcov` behavior — they implement different procedures. The corresponding refit-parity anchor against R's default bootstrap is not yet committed; see the coverage MC (`bootstrap_refit` vs `placebo` tracking) and the Julia `Synthdid.jl` follow-up noted in `TODO.md` for the refit validation surface. Cross-language RNG alignment (Python's PCG64 ≠ R's Mersenne Twister) is why pinned indices are used at all: independently-seeded R and Python bootstraps will not produce equal SE at any finite B.
...
- [x] Bootstrap: fixed weights (original lambda unchanged, omega renormalized for resampled controls). This is a shortcut relative to Arkhangelsky et al. (2021) Algorithm 2 step 2, which specifies re-estimating ω̂_b and λ̂_b per draw; fixed-weight bootstrap underestimates SE relative to the paper's procedure by ignoring weight-estimation uncertainty. **Note (deviation from R):** R's default `synthdid::vcov(method="bootstrap")` also re-estimates per draw (the renormalized ω serves as Frank-Wolfe initialization); our `variance_method="bootstrap"` does not. Use `variance_method="placebo"` (Algorithm 4) or `variance_method="bootstrap_refit"` for paper-faithful and R-default-faithful inference.
- [x] Bootstrap (refit): re-estimates ω̂_b and λ̂_b per draw via two-pass sparsified Frank-Wolfe on the resampled panel (Arkhangelsky et al. 2021 Algorithm 2 step 2 verbatim; also the behavior of R's default `synthdid::vcov(method="bootstrap")`). Opt-in via `variance_method="bootstrap_refit"`. Uses the fit-time normalized-scale zeta. Composed with any survey design raises `NotImplementedError` in this release.
